...
class TreeTopo( Topo ):
    ...
    def build( self, depth=1, fanout=2,queue_size=1000 ):
        # Numbering:  h1..N, s1..M
        self.hostNum = 1
        self.switchNum = 1
        # Build topology
...
def perfTest(queue_size=1000):
    "Create network and run simple performance test"
    # topo = ParkingLotTopo(n=3,delay=delay)
    topo = TreeTopo(depth=4,fanout=2,queue_size=queue_size)
    ...
    # listen on h8
    h1.cmd("iperf3 -s -i 1 > " + "8hop_" + str(queue_size) + "_cubic &")
    h2.cmd("iperf3 -s -i 1 > " + "8hop_" + str(queue_size) + "_bbr &")
    time.sleep(2)
    # start test on h1
    h15.cmd("iperf3 -c 10.0.0.1 -i 1 -t 300 -C cubic > " + "8hop_" + str(queue_size) + "_cubic_sender &")
    time.sleep(50)
    h16.cmd("iperf3 -c 10.0.0.2 -i 1 -t 200 -C bbr > " + "8hop_" + str(queue_size) + "_bbr_sender &")
...
if __name__ == '__main__':
    ...
    for queue_size in range(40,320,40):
        perfTest(queue_size=queue_size)
